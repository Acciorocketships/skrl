...
class MAPPO(Agent):
    ...
    def init(self, trainer_cfg: Optional[Dict[str, Any]] = None) -> None:
        ...
        if self.memory is not None:
            ...
            self.memory.create_tensor(name="log_prob", size=(self.num_agents, 1), dtype=torch.float32, keep_dimensions=True)
            self.memory.create_tensor(name="values", size=(self.num_agents, 1), dtype=torch.float32, keep_dimensions=True)
            self.memory.create_tensor(name="returns", size=(self.num_agents, 1), dtype=torch.float32, keep_dimensions=True)
            self.memory.create_tensor(name="advantages", size=(self.num_agents, 1), dtype=torch.float32, keep_dimensions=True)
    ...
    def _update(self, timestep: int, timesteps: int) -> None:
        ...
        def compute_gae(rewards: torch.Tensor,
            ...
            for i in reversed(range(memory_size)):
                ...
                advantage = rewards[i] - values[i] + discount_factor * not_dones[i].unsqueeze(-1) * (next_values + lambda_coefficient * advantage)
